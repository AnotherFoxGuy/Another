...
var maxForwardSneakSpeed : float = 1.0;
...
function MaxSpeedInDirection (desiredMovementDirection : Vector3) : float {
	var CurrentmaxForwardSpeed = movement.maxForwardSpeed;
	if (Input.GetButton("Sprint"))
		CurrentmaxForwardSpeed = movement.maxForwardSprintSpeed;
	if(Playercontroller.height < 0.5)
		CurrentmaxForwardSpeed = movement.maxForwardSneakSpeed;
	if (desiredMovementDirection == Vector3.zero)
		return 0;
	else {
		var zAxisEllipseMultiplier : float = (desiredMovementDirection.z > 0 ? CurrentmaxForwardSpeed : movement.maxBackwardsSpeed) / movement.maxSidewaysSpeed;
...
private var RotVar = 0;
private var RotTo = "0";//	Left	Right

function UpdateLean(){
	var RotSpeed = Time.deltaTime * 150;
	if(Input.GetButton("Lean Left")){ //	Q
		RotTo = "Left";
		if(RotVar < 20){
			RotVar+= RotSpeed ;
			this.transform.eulerAngles = Vector3(0, this.transform.eulerAngles.y, RotVar);
		}
		else{
			this.transform.eulerAngles = Vector3(0, this.transform.eulerAngles.y, 20);
		}
	}
	else if(Input.GetButton("Lean Right")){ //	E
		RotTo = "Right";
		if(RotVar > -20){
			RotVar-= RotSpeed ;
			this.transform.eulerAngles = Vector3(0, this.transform.eulerAngles.y, RotVar);
		...
		if(RotTo == "Left" && RotVar > 1){
			RotVar-= RotSpeed ;
			this.transform.eulerAngles = Vector3(0, this.transform.eulerAngles.y, RotVar);
		}
		else if(RotTo == "Right" && RotVar < -1){
			RotVar+= RotSpeed ;
			this.transform.eulerAngles = Vector3(0, this.transform.eulerAngles.y, RotVar);
		}
